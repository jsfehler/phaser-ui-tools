...
uiWidgets.Scrollbar.prototype.create = function () {
	...
	// Set default position for the bar.
	this.resetPosition();

	this.valueRange = new uiWidgets.ValueRange(attributes.viewportSize, 0, attributes.containerSize);
...
/** Sets default position for the bar. Center it on the non-moving axis inside the track. */
uiWidgets.Scrollbar.prototype.resetPosition = function () {
	"use strict";
	var x, y;
	
	if (this.vertical) {
		x = this.track.x + (this.track.width / 2) - (this.bar.width / 2);
		y = this.track.y;
	} else {
		x = this.track.x;
		y = this.track.y + (this.track.height / 2) - (this.bar.height / 2);
	}
